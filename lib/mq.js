...
module.exports = class Mq extends EventEmitter {
  constructor() {
    ...
    if(process.env.EGG_SERVER_ENV === 'prod'){
      this.pBaseConfig = {
        ...
        'security.protocol': 'sasl_plaintext',
        'sasl.username': process.env.PDUSERNAME,
        'sasl.password': process.env.PDPASSWORD,
        'sasl.mechanisms': 'SCRAM-SHA-256',
        dr_cb: true,
      };
      this.cBaseConfig = {
        ...
        'group.id': 'handsOfDavinciConsumerGroup',
        'metadata.broker.list': '172.20.249.96:9092,172.20.249.91:9092,172.20.249.95:9092,172.20.249.97:9092',
        'sasl.username': process.env.CDUSERNAME,
        'sasl.password': process.env.CDPASSWORD,
        'sasl.mechanisms': 'SCRAM-SHA-256',
        'security.protocol': 'sasl_plaintext',
        'enable.auto.commit': false
      }
  ...
  _write(_data_str){
    const value = Buffer.from(_data_str);
    this._producerInstance.produce(this.topicName, -1, value, null, Date.now(), function(err, offset) {
      // The offset if our acknowledgement level allows us to receive delivery offsets
      if (err) {
        console.error('本次发送失败咯，', err);
      } else {
        console.log(`${value}对应的${offset}`);
      }
    })
  }

  _start() {
    const interval = Math.random() * 10000;
    setTimeout(() => {
      this.emit('changed',{});
      this._start();
    }, interval);
  ...
  initialConsumer(){
    ...
    this._consumerInstance.on('data', function(m) {
      // Output the actual message contents
      console.log(`Output the actual message contents`, JSON.stringify(m));
    });
  }
};